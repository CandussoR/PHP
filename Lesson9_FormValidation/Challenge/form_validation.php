...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if (in_array($raw_name, $existing_users)) {
    $validation_error .= "This name is already taken.<br>";
  } else {
    $name = $raw_name;
  }
  ...
  $raw_email = filter_var($_POST["email"], FILTER_VALIDATE_EMAIL);
  if ($raw_email) {
    $email = $raw_email;
  } else {
  ...
  $options = [ "options" => ["min_range" => date('Y') - 123, "max_range" => date('Y')]];
